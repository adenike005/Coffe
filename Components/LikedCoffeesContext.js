...
export const useLikedCoffees = () => useContext(LikedCoffeesContext);

export const LikedCoffeesProvider = ({ children }) => {
  ...
  const removeLikedCoffee = (coffeeId) => {
    setLikedCoffees(likedCoffees.filter(coffee => coffee.id !== coffeeId));
  };

  return (
    <LikedCoffeesContext.Provider value={{ likedCoffees, addLikedCoffee, removeLikedCoffee }}>
      {children}
    </LikedCoffeesContext.Provider>
  );
};
